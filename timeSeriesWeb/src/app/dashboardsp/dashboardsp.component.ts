...
@Component({
    selector: 'app-dashboardsp',
    templateUrl: './dashboardsp.component.html',
    styleUrls: ['./dashboardsp.component.css']
})
export class DashboardSpComponent implements OnInit {
    ...
    async getReadingAsync() {
        ...
        await this.commonService.getAsync('reading/getreadingsplist?buildingId=' + this.selectedBuilding + '&objectId=' + this.selectedObject + '&datafieldId=' + this.selectedDataField + '&startDateTime=' + this.dateFormating(this.selectDate[0]) + '&endDateTime=' + this.dateFormating(this.selectDate[1]))
            .then(response => {
